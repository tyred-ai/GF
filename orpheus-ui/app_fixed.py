#!/usr/bin/env python3
"""
Orpheus TTS Web UI Server - Fixed Version
...
# Set environment variable to use CPU for SNAC decoder to save GPU memory
...
async def initialize_model():
    ...
    async with model_lock:
        if model is None:
            ...
            print("Note: SNAC decoder will use CPU to save GPU memory")
...
def generate_audio_sync(model, text, voice):
    ...
    try:
        ...
        for chunk in audio_generator:
            if time.time() - start_time > timeout:
                print(f"Generation timeout after {timeout} seconds")
                break
            ...
            if isinstance(chunk, bytes):
                ...
                try:
                    ...
                    print(f"Warning: Could not convert chunk {chunk_count} to bytes")
            
            if chunk_count % 10 == 0:
                print(f"  Generated {chunk_count} chunks, {len(audio_bytes)} bytes...")
        
        print(f"Final: {chunk_count} chunks, {len(audio_bytes)} bytes")
        return audio_bytes
    ...
    try:
        ...
        while True:
            ...
            if data.get("type") == "generate":
                ...
                if audio_bytes:
                    # Send as single chunk for now (could be improved to stream)
                    await websocket.send_json({
                        "type": "audio_chunk",
                        "session_id": session_id,
                        "chunk_index": 1,
...
if __name__ == "__main__":
    ...
    # Run the server
    print("🚀 Starting Orpheus TTS Enterprise UI Server (Fixed)")
    print("📍 Access at: http://localhost:8000")
    print("⚠️  Note: Using CPU for SNAC decoder to save GPU memory")
